...
export const chapterLessons = {
  "robot-architecture": {
    goal: "能够把一个机器人需求拆成边界清晰、可替换、可测试的软件模块，而不是把所有逻辑堆进节点。",
    details: ["驱动层只处理硬件协议与原始数据；能力层封装定位、抓取等可复用能力；任务层编排流程；应用层面向用户和业务。", "数据流回答信息如何移动，控制流回答谁触发谁。两者要分开画，才能发现循环依赖和隐藏耦合。", "使用抽象接口和注册表按配置创建模块，让型号、算法版本或部署方式变化时不侵入业务代码。"],
    pitfalls: ["按团队成员而不是职责划分模块，最终接口仍然混乱", "让任务层直接访问串口、CAN 或厂商 SDK"],
    quiz: ["更换机械臂品牌时，理想情况下应该改动哪些层？", "为什么数据依赖方向通常应该由上层指向抽象，而不是具体实现？"],
  },
  "robot-perception": {
    goal: "能建立带时间、坐标和质量评估的感知链路，让识别结果真正可用于机器人控制。",
    details: ["先理解各传感器的观测量、频率、噪声和失效模式，再决定互补关系，不能只因为设备可用就全部融合。", "融合前必须统一时间基准和坐标基准；外参描述传感器之间的刚体关系，时间偏差会把运动误认为空间误差。", "滤波用于抑制噪声，融合用于综合多源信息。应同时输出估计值和置信度，避免下游盲目信任。"],
    pitfalls: ["只校准空间外参，不检查时间同步", "把平滑后的曲线当作精度提升，却忽略滤波延迟"],
    quiz: ["机器人运动时点云出现重影，优先检查哪些因素？", "传感器频率高是否一定意味着控制效果更好？"],
  },
  "robot-fsm": {
    goal: "能够设计可预测、可恢复、可测试的任务状态机，并清楚哪些数据必须放入 Context。",
    details: ["状态表示稳定阶段，事件表示外部或内部事实，守卫条件决定转移是否允许，动作只执行转移相关副作用。", "主状态机管理业务阶段，子状态机封装可复用流程；Context 保存跨状态需要的数据、重试次数和恢复依据。", "所有等待都要有超时，所有重试都要有上限，异常路径必须进入已定义的安全状态而不是留在半完成状态。"],
    pitfalls: ["把每个函数调用都设计成一个状态，导致状态爆炸", "依赖全局变量保存状态，重启后无法判断真实进度"],
    quiz: ["Context 与普通局部变量的边界是什么？", "什么时候应该拆成子状态机而不是继续增加主状态？"],
  },
  "robot-control": {
    goal: "能从任务目标逐层得到可执行轨迹，并用反馈数据评价控制器是否稳定、准确。",
    details: ["任务规划决定做什么，路径规划决定经过哪里，轨迹规划再为路径加入时间、速度和加速度约束。", "位置环消除位置误差，速度环改善动态响应，电流或力矩环直接影响执行器输出；内环通常更快。", "前馈依据模型预先补偿，反馈依据误差修正。分析控制效果要同时看稳态误差、超调、带宽和扰动恢复。"],
    pitfalls: ["只比较最终位置，不观察运动过程中的速度和误差", "轨迹点位置连续但速度不连续，导致机械冲击"],
    quiz: ["路径和轨迹的核心区别是什么？", "增加控制增益为什么可能让系统振荡？"],
  },
  "robot-safety": {
    goal: "能用风险和失效模式设计多层安全机制，使单点故障不会直接演化为人员或设备事故。",
    details: ["物理急停必须独立切断危险能量；软件停机用于受控减速和保存状态，两者不能互相替代。", "对碰撞、越界、通信丢失和传感器异常分别定义检测源、判定阈值、响应时间与安全动作。", "故障等级决定是否继续、降级或停机；恢复条件必须比触发条件更严格，避免系统在故障边缘反复切换。"],
    pitfalls: ["把急停按钮只接入普通软件 Topic", "记录了错误码，却没有定义故障恢复的前置条件"],
    quiz: ["为什么安全链路不能只依赖主控计算机？", "哪些故障适合自动恢复，哪些必须人工确认？"],
  },
  "ros2-core": {
    goal: "能根据通信语义正确选择 ROS 2 接口，并设计长期兼容、易调试的消息结构。",
    details: ["Topic 用于持续数据流；Service 用于短时请求响应；Action 用于耗时、可反馈、可取消的任务。选择依据是语义，不是代码方便。", "接口先定义单位、坐标系、时间戳、有效范围和错误语义，再定义字段。避免把所有内容塞进字符串或 JSON。", "命名空间隔离多设备，参数描述部署差异，Launch 负责组装系统；三者共同决定同一代码如何复用。"],
    pitfalls: ["用 Service 执行长时间机械臂动作，无法反馈和取消", "自定义消息缺少时间戳、单位或 frame_id"],
    quiz: ["遥操作关节流应该用 Topic 还是 Action？为什么？", "什么情况下 Service 比 Topic 更合适？"],
  },
  "ros2-qos": {
    goal: "能为不同数据选择 QoS 和执行模型，并用测量数据解释延迟、丢帧与吞吐之间的权衡。",
    details: ["Reliability 决定是否重传，Durability 决定晚加入者能否收到历史数据，History 与 Depth 决定缓存策略。", "Executor 调度回调，Callback Group 约束回调是否可并行；多线程只有在回调组和共享数据设计正确时才产生收益。", "高频传感器常优先新鲜度而非完整性。缓存过深会把短时拥塞变成长延迟，必须结合实时需求压测。"],
    pitfalls: ["所有 Topic 一律 Reliable，网络抖动时延迟不断累积", "使用多线程执行器，却把所有回调放在互斥回调组"],
    quiz: ["相机画面为什么常使用 Best Effort + Keep Last(1)？", "多线程 Executor 什么时候反而会更慢？"],
  },
  "ros2-tf": {
    goal: "能读懂并构建 TF 树，正确完成任意传感器、机器人基座和工具之间的时空变换。",
    details: ["树中的边表示父坐标系到子坐标系的位姿；查询时先写清源坐标系和目标坐标系，避免凭名称猜乘法方向。", "固定安装关系发布静态 TF，运动关节发布动态 TF。查询动态 TF 必须考虑消息时间，而不是总取最新值。", "URDF 的 link 表示刚体，joint 表示约束关系；visual 负责显示，collision 负责碰撞计算，两者可以不同。"],
    pitfalls: ["把物体坐标值直接当成 base_link 下的坐标使用", "TF 发布频率正常，但时间戳来自错误时钟"],
    quiz: ["已知点在 camera_link 下的坐标，如何转换到 base_link？", "为什么 TF 树中一个子坐标系不能有两个父节点？"],
  },
  "ros2-runtime": {
    goal: "能把 ROS 2 节点组织成可控启动、低开销运行、故障可诊断的工程系统。",
    details: ["Component 让多个节点运行在同一进程并支持进程内通信，但也共享故障域，需要权衡性能和隔离性。", "Lifecycle 把创建、配置、激活和停机显式化，适合有硬件依赖和启动顺序的节点。", "参数描述可变配置，Launch 描述系统组合，诊断与日志描述运行健康；不要把业务状态藏在参数中。"],
    pitfalls: ["为了省进程把所有节点放进一个组件容器，导致单点崩溃", "节点构造函数中直接启动硬件，无法受控配置和恢复"],
    quiz: ["哪些节点适合组件化，哪些应保持进程隔离？", "Lifecycle 的 inactive 状态应该允许哪些行为？"],
  },
  "ros2-stack": {
    goal: "能理解 Nav2、MoveIt 2 与 ros2_control 的职责边界，并把移动、规划和执行连接成闭环。",
    details: ["Nav2 通过行为树编排导航，规划器产生路径，控制器跟踪路径，代价地图表达静态和动态障碍。", "MoveIt 2 在 Planning Scene 中检查机器人与环境碰撞，生成关节轨迹，但真实执行由控制器完成。", "ros2_control 用硬件接口隔离设备差异，Controller Manager 管理控制器生命周期和资源占用。"],
    pitfalls: ["把 MoveIt 2 当作底层电机控制器", "规划成功后直接认为任务成功，不检查实际执行结果"],
    quiz: ["Nav2 全局规划器和局部控制器分别解决什么问题？", "MoveIt 2 与 ros2_control 之间传递的核心数据是什么？"],
  },
  "kin-coordinates": {
    goal: "建立坐标表示的直觉，能区分几何对象本身与它在某个坐标系中的数值表示。",
    details: ["点表示位置，向量表示方向和大小，基决定坐标数字的含义；同一个几何向量在不同基下坐标不同。", "矩阵乘向量可看作变换，也可看作基向量线性组合；理解这一点比记公式更重要。", "主动变换改变物体，被动变换改变观察坐标系；数值公式相似但物理含义和旋转方向相反。"],
    pitfalls: ["把坐标数值当作与坐标系无关的绝对位置", "只背矩阵公式，不标注向量属于哪个坐标系"],
    quiz: ["同一个点换坐标系后，什么改变、什么不变？", "为什么矩阵 AB 与 BA 通常表示不同变换？"],
  },
  "kin-rotation": {
    goal: "能在旋转矩阵、欧拉角和四元数之间转换，并理解每种表示的约束与局限。",
    details: ["旋转矩阵的列向量是旋转后坐标轴，满足正交且行列式为 1；它直观但有 9 个数和 6 个约束。", "欧拉角紧凑易读，但旋转顺序不可省略，并在特定姿态出现万向锁。", "单位四元数用半角编码旋转，必须归一化；q 与 -q 表示同一姿态，插值时要选择最短路径。"],
    pitfalls: ["混用 XYZ 固定轴旋转和 XYZ 随体轴旋转", "把非单位 xyzw 直接作为合法姿态"],
    quiz: ["绕 Z 轴 90° 的四元数为什么使用 45° 的正弦和余弦？", "如何验证一个 3×3 矩阵是不是旋转矩阵？"],
  },
  "kin-transform": {
    goal: "能用齐次矩阵表达刚体位姿，正确组合坐标链并验证变换方向。",
    details: ["齐次矩阵上左角是旋转，右上角是平移，最后一行使旋转和平移可以统一用矩阵乘法组合。", "矩阵连乘从右向左作用；平移量必须表达在对应父坐标系中，不能只看数值。", "逆变换的旋转是转置，平移是负的旋转转置乘原平移；它不是简单把所有元素取负。"],
    pitfalls: ["坐标链顺序正确但每个矩阵的方向标注错误", "把平移和旋转交换顺序，得到不同物理结果"],
    quiz: ["为什么齐次矩阵需要扩展为 4×4？", "如何不用完整矩阵求逆就得到刚体变换的逆？"],
  },
  "kin-fk": {
    goal: "能从机械臂几何结构建立模型，并计算任意关节角对应的末端位姿。",
    details: ["DH 方法通过相邻关节轴建立坐标系，用四个参数描述两个坐标系之间的标准变换。", "标准 DH 与改进 DH 的坐标系附着和变换次序不同，参数不能直接混用。", "正运动学是逐段变换连乘；验证时要覆盖零位、典型姿态和随机姿态，并与 URDF 或仿真对照。"],
    pitfalls: ["为得到好看的参数而错误放置关节坐标系", "只验证零位，隐藏的符号和轴向错误没有暴露"],
    quiz: ["DH 四个参数中哪些通常是关节变量？", "正运动学为什么通常容易，而逆运动学更难？"],
  },
  "kin-ik": {
    goal: "能求解目标位姿对应的关节角，并识别不可达、关节限位与奇异问题。",
    details: ["解析 IK 速度快且能枚举分支，但依赖特定结构；数值 IK 通用，通过迭代减小位姿误差。", "Jacobian 把关节速度映射到末端速度；奇异位置会丢失某些运动方向并放大关节速度。", "阻尼最小二乘用小偏差换取奇异附近的稳定性，还应加入关节限位、步长和终止条件。"],
    pitfalls: ["只判断位置误差，不处理姿态误差的表示", "迭代未收敛仍返回最后一组关节角作为成功结果"],
    quiz: ["Jacobian 的每一列在几何上表示什么？", "阻尼系数过大和过小分别会带来什么问题？"],
  },
  "emb-cpp": {
    goal: "能编写资源安全、并发可控、便于定位问题的机器人 C++ 程序。",
    details: ["RAII 把资源生命周期绑定到对象；优先使用值语义和智能指针，明确所有权后再选择共享。", "进程隔离强，线程共享地址空间；锁保护不变量，条件变量用于等待状态变化，不能靠轮询代替同步。", "Linux 把设备和许多接口抽象成文件；理解 fd、阻塞模式、poll/epoll 和信号有助于写稳定驱动。"],
    pitfalls: ["用 shared_ptr 掩盖所有权设计不清", "持锁执行阻塞 I/O，导致其他线程长期无法运行"],
    quiz: ["unique_ptr 与 shared_ptr 的选择标准是什么？", "条件变量为什么必须与条件谓词一起使用？"],
  },
  "emb-bus": {
    goal: "能根据物理链路和实时需求选择协议，并写出面对异常数据仍可恢复的通信层。",
    details: ["UART 是字节流，RS485 解决差分多点物理传输，Modbus RTU 再在其上定义地址、功能码和校验。", "CAN 通过标识符仲裁并天然广播；CAN FD 扩大载荷和数据段速率，但节点兼容性必须确认。", "TCP 保证有序字节流但没有消息边界，UDP 保留报文但不保证到达；应用层仍需序列号、超时和状态机。"],
    pitfalls: ["认为一次 TCP recv 就对应一帧完整消息", "只处理正确帧，没有丢字节后的重新同步策略"],
    quiz: ["RS485、Modbus RTU 和串口之间是什么关系？", "CAN 总线高优先级报文过多会发生什么？"],
  },
  "emb-rtos": {
    goal: "能为实时任务分配优先级和通信机制，并证明关键任务在最坏情况下仍按时完成。",
    details: ["中断只做必须的快速响应，把耗时处理转交任务；任务优先级应反映截止时间和阻塞关系。", "互斥锁保护共享资源，信号量表示资源计数，队列传递数据和所有权，事件组适合组合状态通知。", "实时性关注最坏延迟而非平均速度；需要估算 WCET、阻塞时间、抖动和 CPU 利用率。"],
    pitfalls: ["在高优先级任务中打印大量日志", "所有任务优先级凭经验设置，没有测量最坏执行时间"],
    quiz: ["中断服务函数中为什么不应执行复杂业务？", "优先级反转如何产生，常见解决方法是什么？"],
  },
  "emb-driver": {
    goal: "能把不稳定的硬件链路封装成稳定、可观测、可替换的软件接口。",
    details: ["HAL 隔离平台差异，协议层解析帧，设备层维护连接和配置状态，发布层把数据转换为系统接口。", "采集线程要有明确周期、缓存边界和退出机制；滤波参数必须与采样频率匹配，标定参数要可追溯。", "硬件错误不能只打印日志，应映射成诊断状态、错误码和恢复动作，并统计连续失败与累计失败。"],
    pitfalls: ["把串口读取、协议解析和业务发布写在同一个无限循环", "设备重连后没有重新下发配置和验证版本"],
    quiz: ["驱动层应该向上层暴露原始错误还是统一错误码？", "如何判断丢帧发生在硬件、驱动还是发布环节？"],
  },
  "emb-reliable": {
    goal: "能让机器人服务在边缘设备上长期运行，并在崩溃、断电和升级失败后可恢复。",
    details: ["部署包要固定依赖、配置和模型版本，区分只读程序、可变配置、数据与日志目录。", "systemd 管理进程生命周期，健康检查判断服务是否真正可用，日志轮转防止磁盘耗尽。", "升级需要版本校验、兼容性检查、原子切换和回滚；故障恢复后要保留导致异常的上下文。"],
    pitfalls: ["进程仍在就认为服务健康", "日志无限增长，最终使磁盘写满导致全系统异常"],
    quiz: ["进程存活检查与业务健康检查有什么区别？", "怎样设计升级过程才能在断电后仍保持可启动？"],
  },
  "llm-base": {
    goal: "能解释大模型推理的关键结构和资源消耗，不把模型能力简单归因于参数量。",
    details: ["Token 是模型处理的离散单元，Embedding 把它映射为向量；位置编码让注意力知道顺序。", "Q 表示当前元素寻找什么，K 表示元素提供什么索引，V 表示实际聚合的信息；注意力按相关性加权。", "预训练学习通用模式，指令微调塑造交互行为；推理时上下文和 KV Cache 决定显存与响应速度。"],
    pitfalls: ["把 Token 数量等同于字符数量", "只看模型参数显存，忽略长上下文的 KV Cache"],
    quiz: ["Self-Attention 为什么能并行处理输入序列？", "上下文长度增加时推理成本主要如何变化？"],
  },
  "llm-serving": {
    goal: "能部署并测量本地大模型服务，在显存、吞吐、延迟和稳定性之间做工程权衡。",
    details: ["量化降低权重显存与带宽，但不同算法对速度和精度影响不同；视觉模型还要考虑图像编码器。", "vLLM 用 PagedAttention 管理 KV Cache，用连续批处理动态合并请求，提高 GPU 利用率。", "TTFT 衡量首字等待，TPS 衡量生成速度；并发提高总吞吐但会增加排队和单请求延迟。"],
    pitfalls: ["只做单请求测速，无法反映真实服务能力", "显存刚好装满模型，没有为 KV Cache 和峰值保留空间"],
    quiz: ["吞吐量和单请求延迟为什么可能同时变差或互相冲突？", "张量并行何时能提升性能，何时只增加通信开销？"],
  },
  "llm-rag": {
    goal: "能构建可评测、可引用、能定位失败环节的知识问答系统。",
    details: ["解析先保留标题、章节、表格和来源元数据；切分要围绕语义完整性与检索粒度权衡。", "向量检索擅长语义相近，关键词检索擅长专有名词和编号，重排器再对候选做精细相关性判断。", "评测要分开衡量检索召回、引用准确和答案正确；GraphRAG 适合关系密集、需要跨文档推理的知识。"],
    pitfalls: ["切分后丢失文档标题和章节路径", "答案看似合理就认为 RAG 成功，没有核对引用是否支持结论"],
    quiz: ["什么时候混合检索明显优于纯向量检索？", "如何区分没有检索到证据与模型忽略了证据？"],
  },
  "llm-agent": {
    goal: "能设计受约束、可追踪的 Agent，让模型可靠选择工具而不是直接执行不可控动作。",
    details: ["工具描述要清楚输入、输出、前置条件和副作用；结构化参数必须经过类型和业务规则校验。", "MCP 统一模型与工具的发现和调用，Registry 管理能力，工作流负责确定性流程，模型只处理需要判断的部分。", "记忆保存长期有用信息，状态保存当前任务进度；每次计划、调用、结果和错误都应形成可审计轨迹。"],
    pitfalls: ["把所有流程都交给模型自由规划，缺少确定性边界", "工具调用失败后无限重试或悄悄换参数"],
    quiz: ["哪些机器人动作必须在工具层再次校验，而不能信任模型参数？", "工作流与 Agent 自主规划应该如何分工？"],
  },
  "llm-vla": {
    goal: "能设计模型决策与高精度工具协作的具身系统，并用闭环指标验证真实任务能力。",
    details: ["VLM 理解图像和语言，VLA 把观测映射为动作；任务分解、坐标精度和安全约束通常仍需工程模块补强。", "低精度、容错高的动作可用端到端策略；高精度任务应调用检测、测量、标定、规划和伺服技能。", "模型选择技能，执行层施加速度、碰撞和工作空间约束，感知层验证结果；失败必须反馈给决策层重新规划。"],
    pitfalls: ["把 VLA 输出直接发送给执行器，绕过安全控制", "只展示一次成功案例，不统计精度、成功率和失败分布"],
    quiz: ["什么指标可以判断任务应该走端到端策略还是高精度工具链？", "VLA 闭环中哪一层负责确认动作确实达到目标？"],
  },
};
